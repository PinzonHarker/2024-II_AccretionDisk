...
!=======================================================================
  ...
  use Gravity_data, ONLY: grv_ptxpos, grv_ptypos, grv_ptzpos, grv_factor, grv_rs, &
       useGravity
...
!==============================================================================
  ...
  if (sweepDir .eq. SWEEP_X) then               ! x-component
     ...
     tmpdr32 = yCenter(j)*yCenter(j) + zCenter(k)*zCenter(k) + grv_rs*grv_rs
     ...
     tmpdr32 = xCenter(j)*xCenter(j) + zCenter(k)*zCenter(k) + grv_rs*grv_rs
     ...
     tmpdr32 = xCenter(j)*xCenter(j) + yCenter(k)*yCenter(k) + grv_rs*grv_rs
