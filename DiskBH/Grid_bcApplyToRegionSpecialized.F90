!!****if* source/Simulation/SimulationMain/DiscoNSRefRot/Grid_bcApplyToRegionSpecialized
!!
!!
!! NAME
...
#include "Flash.h"
  ...
  use Simulation_data,  ONLY : sim_mas, sim_temp, sim_dotmas, & 
                               sim_k, sim_mash, sim_g, &
                               sim_g, sim_mu, sim_smallP, &
                               sim_gamma, sim_ymax, sim_xmax 
  ...
  real    :: tempRho, tempVr, tempVphi, tempPres
           ...
           if(face==LOW) then ! apply BC on the bottom boundary at x=xmin
                 ...
                 do i = 1,guard
                    ...
                    if (sign == -1) then
                       do n=1,ke
                          do j=1,je
                             regionData(i,j,n,ivar) = min(regionData(i,j,n,ivar),0.0)
                             if (ivar == VELY_VAR) regionData(guard+1,1:je,1:ke,ivar) = 2.46E10
                          end do
                       end do
                    end if
                 end do
              ...
              case(USER_DEFINED)
                 ...
                 do i = 1,guard
                    do j=1,je
                       if (gridDataStruct==CENTER) then
                       	  tempRho = sim_dotmas/(4.0*3.1416*1.0E-4*(xCoord(i+k+xOffset-1))**2*sqrt(sim_g*sim_mas/ &
				xCoord(i+k+xOffset-1)))
                          tempVr = -0.01*sqrt(sim_g*sim_mas/xCoord(i+k+xOffset-1))
                          tempVphi = sqrt(sim_g*sim_mas/xCoord(i+k+xOffset-1))
                          tempPres = (tempRho*sim_k*sim_temp)/(sim_mu*sim_mash)
                          if (ivar == DENS_VAR) regionData(k+i,j,1:ke,ivar) = tempRho
                          if (ivar == VELX_VAR) regionData(k+i,j,1:ke,ivar) = tempVr
                          if (ivar == VELY_VAR) regionData(k+i,j,1:ke,ivar) = tempVphi
                          if (ivar == VELZ_VAR) regionData(k+i,j,1:ke,ivar) = 0.
                          if (ivar == PRES_VAR) regionData(k+i,j,1:ke,ivar) = tempPres
                          if (ivar == GAMC_VAR) regionData(k+i,j,1:ke,ivar) = sim_gamma
                          if (ivar == GAME_VAR) regionData(k+i,j,1:ke,ivar) = sim_gamma
                          if (ivar == EINT_VAR) regionData(k+i,j,1:ke,ivar) = tempPres /tempRho*(sim_gamma-1.0)
                          if (ivar == ENER_VAR) regionData(k+i,j,1:ke,ivar) = tempPres /(tempRho*(sim_gamma-1.0)) &
				+ 0.5*(tempVr**2 + tempVphi**2)
                       endif
                    end do
                 end do
              case default
